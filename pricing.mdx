...
- Proportional: 35 bps (0.35%) of the amount

**Formula:**

```
protocol_fee = floor(amount × 35 / 10_000)
```
...
protocol_fee = floor(1,000,000,000 × 35 / 10,000) = 3,500,000 µUSDC = 3.50 USDC

net received = 1,000,000,000 - 3,500,000 = 996,500,000 µUSDC = 996.50 USDC
```
...
function estimateProtocolFee(amount: bigint): bigint {
  return (amount * PROTOCOL_FEE.BPS) / BPS_DIVISOR;
}

const fee = estimateProtocolFee(1_000_000n); // 1 USDC
// fee = 1_000_000n × 35n / 10_000n = 3_500n µUSDC (0.0035 USDC)
```
...
```
relayer_fee = RELAYER_BASE_FEE + floor(amount × RELAYER_BPS / 10_000)
...
Umbra deducts:           protocol_fee = floor(actual_received × 35 / 10,000)
