...
### Example — 1,000 USDC ETA → PATA withdrawal
...
- Self-deposit (PATA → own ETA): 0 protocol fee for standard self-shielding.
- Withdrawal (ETA → PATA): protocol fee applies.
...
- Note creation (PATA → Stealth Pool Note): protocol fee on SPL amount + Stealth Pool SOL fee.
- Burn (note → ETA): protocol fee applies, relayer fee applies (currently 0).
- Burn (note → PATA): protocol fee applies, relayer fee applies (currently 0). (Only the self-burnable → PATA variant is shipped in V14; receiver-burnable → PATA is on-chain but not yet exposed in the SDK.)
