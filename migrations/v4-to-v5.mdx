...
<Note>
**Current release: `@umbra-privacy/sdk@5.0.0-rc.6`** (pins `@umbra-privacy/umbra-codama@3.0.0-rc.6`).
Nothing extra to wire up — the earlier `rc.3`/`rc.4` workarounds are gone: the codama
PDA bug (`ConstraintSeeds 2006` on deposit) and the columnar `scan()` BigInt bug are
**both fixed in rc.6**. No package-manager override, no `postinstall` dist patch.
</Note>

## TL;DR

1. Bump the package: `@umbra-privacy/sdk@4.x → @umbra-privacy/sdk@5.0.0-rc.6`. Pin an **exact** version (codama is pinned by the SDK — no override needed).
