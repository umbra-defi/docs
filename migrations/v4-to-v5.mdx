...
description: "Step-by-step migration from @umbra-privacy/sdk@4.x (V13) to @umbra-privacy/sdk@5.x (V14): package consolidation, subpath imports, factory renames, zero-arg scanner, burner relayer-dep shape, store adapters."
---
...
<Warning>
**Current release: `@umbra-privacy/sdk@5.0.0-rc.4`.** It resolves
...
fixed release until the next SDK release bumps it:
...
There is also a known `scan()` bug in `rc.3`–`rc.4` (a `BigInt()` call on a base64
field throws on the columnar fetch path). Until it's fixed upstream, apply a
one-file dist patch (`scripts/patch-sdk.mjs`, wired as a `postinstall` hook) that
rewrites the two lines to decode the bytes correctly.
</Warning>

## TL;DR

1. Bump the package: `@umbra-privacy/sdk@4.x → @umbra-privacy/sdk@5.0.0-rc.4`. Pin an **exact** version, and add the `@umbra-privacy/umbra-codama@3.0.0-rc.4` override (see the warning above).
...
4. Rename factories per the table below (UTXO → Stealth Pool Note, claim → burn, ATA → PATA, encrypted balance → ETA).
...
8. (Mainnet) Mint list adds `CASH`. (Devnet) Mint list drops to wSOL only.
...
| `BPS_DIVISOR`, mint lists | `@umbra-privacy/sdk` | `@umbra-privacy/sdk/constants` |
...
- **`ATA` / `PublicBalance`** → **`PublicAssociatedTokenAccount` (PATA)** in factory names.
...
console.log("Self-burnable from PATA:", result.publicSelfBurnable.length);
console.log("Receiver from PATA:",      result.publicReceived.length);
...
console.log("Receiver from PATA:        ", result.ataToStealthPoolReceiverBurnable.length);
...
V4 claimers returned `{ signatures: Record<number, TransactionSignature[]> }`. V5 burners return `BurnStealthPoolNoteIntoETAResult` (or `…IntoPublicAssociatedTokenAccountResult` for the PATA variant):
...
- **Receiver-burnable → ETA** batches **natively**: groups notes by `destinationAddress`, chunks to ≤5 per proof. Pass the whole array.
- **Self-burnable → ETA / PATA** has `MAX_NOTES_PER_PROOF = 1`. The SDK loops internally — caller still passes an array.
...
const r    = getUmbraRelayer({ apiEndpoint: relayerApiEndpoint });
const burn = getReceiverBurnableStealthPoolNoteIntoETABurnerFunction(
  { client },
  {
    fetchBatchMerkleProof: client.fetchBatchMerkleProof!,
...
- [ ] `getUmbraClient(...)` includes `relayerApiEndpoint` and, on browser, `utxoDataStore` + `nullifierStore`.
...
- [ ] Mainnet mint allowlist includes `CASH`; devnet allowlist is wSOL-only.
...
The V5 surface is the **target shape** going forward. Future minor releases will add factories (e.g. receiver-burnable → PATA), additional store backends, and ergonomic helpers — but the existing V5 factory names, subpath layout, and `relayer` dep shape are stable.
