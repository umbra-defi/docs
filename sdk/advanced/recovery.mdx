---
title: "Recovery"
description: "Deterministic recovery: same wallet + same network recreates all keys. UTXO rescan via getClaimableUtxoScannerFunction(0, 0). Master seed persistence patterns."
---
...
const fetch = getClaimableUtxoScannerFunction({ client });

// Scan tree 0 from the beginning
const result = await fetch(0, 0);

console.log("Recovered self-burnable UTXOs:", result.selfBurnable.length);
console.log("Recovered received UTXOs:", result.received.length);
```
...
async function recoverAllUtxos(treeIndex: number) {
  ...
  while (true) {
    ...
    allUtxos.push(...result.selfBurnable, ...result.received);

    if (result.selfBurnable.length === 0 && result.received.length === 0) {
