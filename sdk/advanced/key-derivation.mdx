...
description: "KMAC256 master seed from Ed25519 wallet signature, full key hierarchy (MVK, Poseidon PK, X25519 keypairs), BN254 field element sampling from 512-bit output."
---
...
```
message = UMBRA_MESSAGE_TO_SIGN   // multi-paragraph legal consent message
                                  // exported from @umbra-privacy/sdk
...
The signing message is a multi-paragraph legal consent text exported as `UMBRA_MESSAGE_TO_SIGN` from `@umbra-privacy/sdk`. It includes the wallet address and a consent statement. The KMAC256 step ensures the seed is uniformly distributed regardless of the signature's internal structure, and adds domain separation to prevent the raw signature from being used as a key elsewhere.
...
│   ├── Mint X25519 Keypair  · Curve25519 · Stealth Pool Note ciphertext (per mint)
│   └── Rescue Blinding Factor  · BN254 · commitment blinding in ZK
├── Poseidon Private Key  · BN254 · Stealth Pool Note nullifiers · user commitment
...
<Warning>
Rotating an offset produces a different public key or nullifier. If you have already registered an X25519 key on-chain or written Stealth Pool Notes, rotating the relevant offset will mean your existing on-chain state uses the old key and the new client uses the new key. Always rotate carefully and re-register if needed.
</Warning>
...
Stealth Pool Note creation operations use **ephemeral master seeds** — one-time seeds derived from the persistent master seed combined with a monotonic counter:
...
The ephemeral seed is used to generate the per-note X25519 keypair and blinding factor, ensuring each Stealth Pool Note is cryptographically independent. The offset comes from the user's generation index, which is incremented on-chain with each operation.
...
**Forward secrecy**: ephemeral seeds for Stealth Pool Notes are keyed on the generation index. Compromising the master seed does not allow deriving future ephemeral seeds without also knowing future generation index values.
