---
title: "ZK Provers"
description: "Custom Groth16 provers: provers ship inside @umbra-privacy/sdk/zk-prover, CDN asset provider, web worker setup with comlink, 2-8s browser / 1-3s Node.js proof generation."
---
...
**The prover ships inside `@umbra-privacy/sdk/zk-prover`.** All factories listed below wrap [snarkjs](https://github.com/iden3/snarkjs) and handle proving-key loading from Umbra's CDN by default.
...
<Note>
**Two naming patterns.** Creator factories use `get{ETA,ATA}IntoStealthPoolNoteCreatorProver`. Burner factories use `getClaim{Self,Receiver}ClaimableUtxoInto{EncryptedBalance,PublicBalance}Prover` — these names come from the wire-protocol vocabulary (`claim` / `utxo`), which the burn pipeline still uses on the relayer endpoint.
</Note>
