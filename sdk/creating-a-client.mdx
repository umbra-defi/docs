...
description: "Call getUmbraClient(args: {signer, network, rpcUrl}) to create an IUmbraClient. Covers per-call commitment override model, deps injection, and client reuse across factory functions."
---
...
<Note>
The relayer endpoint is **not** a `getUmbraClient` argument. For burning, construct a relayer client separately with `getUmbraRelayer({ apiEndpoint: "https://relayer.api.umbraprivacy.com" })` and pass it to the burner factory's `relayer` dep.
</Note>

<ParamField path="deps.utxoDataStore" type="UtxoDataStore">
  Persistent storage for scan cursors and decrypted Stealth Pool Note data. **Strongly recommended on the browser** — without it the scanner re-scans every active tree from genesis on every call. Build it with `await createShardedUtxoDataStore(client, backend)` from `@umbra-privacy/sdk/store-adapters`, where `backend` comes from `createBrowserStorageBackend()`. Because the store derives its encryption keys from `client.masterSeed`, the client must already exist — so either pass it through the `deps` argument once you have an instance, or attach it after construction with `client.utxoDataStore = …` (see the full setup example below).
</ParamField>

<ParamField path="deps.nullifierStore" type="NullifierStore">
  Persistent storage for nullifiers discovered during scanning. Same provenance and recommendation as `utxoDataStore` — build it with `await createShardedNullifierStore(client, backend)`.
</ParamField>
...
// 2. Build the storage backend (browser/IndexedDB — takes no arguments).
const backend = createBrowserStorageBackend();

// 3. Build the sharded stores FROM the client (async — they read
//    client.masterSeed to derive their encryption keys), then attach them.
client.utxoDataStore  = await createShardedUtxoDataStore(client, backend);
client.nullifierStore = await createShardedNullifierStore(client, backend);
```
