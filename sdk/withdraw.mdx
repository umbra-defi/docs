---
title: "Withdraw"
description: "getETAIntoATAWithdrawerFunction (V14): move tokens from an EncryptedTokenAccount to a PublicAssociatedTokenAccount. Returns WithdrawResult with Arcium MPC callback confirmation."
---

## Overview

The `withdraw` operation is the reverse of a deposit. It moves tokens from a user's `EncryptedTokenAccount (ETA)` back to their `PublicAssociatedTokenAccount (PATA)`.

```
ETA (on-chain encrypted balance)
          │
          ▼
  Pool custody (on-chain SPL)  ──withdraw──▶  Your PATA
```
...
<ParamField path="destinationAddress" type="Address" required>
  The Solana address whose PATA will receive the withdrawn tokens. This is typically `client.signer.address`.
</ParamField>
...
<ParamField path="options.feeVaultOffset" type="U128" default="0n">
  Fee-vault PDA offset. The fee-vault path is currently unused for public-PATA withdrawals — leave at the default `0n` (sentinel for "no fee-vault interaction").
</ParamField>
...
<Note>
There are no `priorityFees`, `awaitCallback`, `skipPreflight`, `maxRetries`, or `epochInfoCommitment` options on the V14 withdrawer. Priority fees are configured at factory time. Callback waiting is always-on — the result's `callback` field is populated on success.
</Note>
...
The tokens are sent to the **PATA** associated with the `destinationAddress` for the given mint. If that PATA does not exist, the transaction will fail with an account-not-found error. Create the PATA first using standard token tooling if needed.
