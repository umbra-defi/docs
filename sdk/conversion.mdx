---
title: "Conversion"
description: "getNetworkEncryptionToSharedEncryptionConverterFunction: upgrade MXE-only ETAs to Shared mode. getMintEncryptionKeyRotatorFunction: rotate per-mint X25519 encryption keys (MPC)."
---
...
<ParamField path="hooks" type="ConvertToSharedHooks">
  Optional overall + per-mint lifecycle hooks. Top-level fields cover the batch (`onValidationStart`/`onValidationComplete`, `onAccountFetchStart`/`onAccountFetchComplete`, `onMintConversionStart`/`onMintConversionComplete`/`onMintConversionSkipped`, `onComplete`, `onError`). The MPC-step hooks for each individual conversion (`onPreSend`, `onPostSend`, `onMonitorStarted`, `onFinalized`) live inside the `perMintTransaction` member.
</ParamField>
...
Conversion submits all eligible mints in parallel (via `Promise.all`). If a transaction fails or is cancelled mid-batch, whichever conversions landed before the rejection will already be confirmed on-chain — they are not rolled back. Because submission is parallel, the set of mints that succeeded before a failure is non-deterministic; inspect on-chain state (or re-run `convert()`, which skips already-shared mints) rather than assuming a sequential checkpoint.
  ...
  if (isConversionError(err)) {
    switch (err.stage) {
      case "transaction-sign":
        ...
        // Conversions run in parallel, so some mints may already be confirmed
        // on-chain while others never submitted — the partial set is non-deterministic.
        // Re-call convert() with the same mint list; already-shared mints are skipped.
        showNotification("Conversion cancelled. Re-run to convert any remaining mints.");
        break;
      ...
      case "transaction-send":
        // A per-mint transaction was submitted but confirmation timed out.
        // Other mints in the parallel batch may have landed; the partial set is
        // non-deterministic. Check on-chain before retrying.
