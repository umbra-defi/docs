...
description: "getComplianceGrantIssuerFunction and getComplianceGrantRevokerFunction: create/revoke on-chain PDA grants that authorize Arcium MPC to re-encrypt ETA ciphertexts under a grantee's X25519 key."
---
...
The MVK X25519 key is used to prove ownership of the master viewing key during grant creation. The SDK derives it internally via the `getMasterViewingKeyX25519KeypairDeriver` dependency.
...
const generateMvkKeypair = getMasterViewingKeyX25519KeypairDeriver({ client });
...
const signature = await createGrant(
  ...
  nonce,          // RescueCipherEncryptionNonce - u128 bigint
);
...
```typescript
import { getUserComplianceGrantQuerierFunction } from "@umbra-privacy/sdk/compliance";
...
const result = await queryGrant(
  ...
  nonce,          // RescueCipherEncryptionNonce - the same nonce used at creation
...
```typescript
import { getSharedCiphertextReencryptorForUserGrantFunction } from "@umbra-privacy/sdk/compliance";
...
const signature = await reencrypt(
  ...
  nonce,                // RescueCipherEncryptionNonce - the nonce used when creating the grant
  inputEncryptionNonce, // RescueCipherEncryptionNonce - the nonce of the ciphertext to re-encrypt
...
<ParamField path="nonce" type="RescueCipherEncryptionNonce" required>
  The `u128` bigint nonce from the grant creation. Used to locate the grant PDA on-chain.
</ParamField>

<ParamField path="inputEncryptionNonce" type="RescueCipherEncryptionNonce" required>
...
```typescript
import { getComplianceGrantRevokerFunction } from "@umbra-privacy/sdk/compliance";
...
const signature = await deleteGrant(
  ...
  receiverX25519, // X25519PublicKey - receiver's X25519 key
  nonce,          // RescueCipherEncryptionNonce - must be the original nonce
);
...
const generateMvkKeypair = getMasterViewingKeyX25519KeypairDeriver({ client: granterClient });
const mvkKeypairResult = await generateMvkKeypair();
const granterX25519 = mvkKeypairResult.x25519Keypair.publicKey;
