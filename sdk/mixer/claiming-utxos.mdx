---
title: "Burning Notes"
description: "3 burner factory functions: receiver-burnable → ETA, self-burnable → ETA, self-burnable → ATA. Each needs a fetchBatchMerkleProof, a circuit-specific zkProver, and a relayer adapter ({submitBurn, pollBurnStatus, getRelayerAddress})."
---
...
- **Into an AssociatedTokenAccount** — tokens become visible in the destination ATA. Self-burnable only.
...
<ParamField path="relayer" type="{ submitBurn; pollBurnStatus; getRelayerAddress }" required>
  Relayer adapter. The three property names (`submitBurn`, `pollBurnStatus`, `getRelayerAddress`) are TypeScript aliases of the relayer client's claim methods (`BurnSubmitterFunction = ClaimSubmitterFunction`, etc.), so a relayer client built via `getUmbraRelayer({ apiEndpoint })` plugs in directly.
</ParamField>
...
interface BurnBatchResult {
  ...
  readonly resolvedVariant?: string;                                // relayer-chosen variant, e.g. "claim_into_existing_shared_balance_v17"
...
<Note>
**Umbra Confidential SPL lottery points:** When burning into an ETA, the Arcium MPC generates a `rescue_encrypted_lottery_ticket_delta` and adds it to your ETA's running point totals. This field is present in the raw relayer batch response but is not yet surfaced in `BurnBatchResult`. See [Lottery Points (Umbra Confidential SPL)](/concepts/encrypted-balances#lottery-points-umbra-confidential-spl) for the current SDK status.
</Note>

## Batching behaviour

- **Receiver-burnable → ETA** batches **natively**: groups notes by `destinationAddress`, chunks to ≤4 per proof. Pass the whole array; do not reimplement chunking.
- **Self-burnable → ETA / ATA** has `MAX_STEALTH_POOL_NOTES_PER_PROOF = 1`. The SDK loops internally — caller still passes an array; the result has one batch per note.
...
Merkle proofs can become stale if the tree root changes between when the proof is fetched and when the burn is submitted. The burner fetches the proof **per batch, immediately before submission**, which keeps the window small. If a burn fails with a root mismatch, simply re-run it — the burner refetches the proof on each attempt.
