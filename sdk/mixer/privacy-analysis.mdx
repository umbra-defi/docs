...
description: "3-tier privacy model: ETA → ETA (strongest), mixed flows (partial), ATA → ATA (weakest). Covers timing analysis, amount correlation, anonymity set sizing, and self-burnable vs receiver-burnable trade-offs."
---
...
- **Source** — where the tokens come from: a `AssociatedTokenAccount (ATA)` or an `EncryptedTokenAccount (ETA)`.
...
- **Tier 2** — Mixed (one end shielded)
  - ETA → receiver-burnable → ATA *(not yet shipped in SDK; on-chain only)*.
  - ETA → self-burnable → ATA.
  - ATA → receiver-burnable → ETA.
  - ATA → self-burnable → ETA.
- **Tier 3** — ATA → ATA (both ends public)
  - ATA → receiver-burnable → ATA *(not yet shipped in SDK)*.
