...
description: "4 note-creator factory functions: get{ATA|ETA}IntoSelfBurnableStealthPoolNoteCreatorFunction and get{ATA|ETA}IntoReceiverBurnableStealthPoolNoteCreatorFunction. Merkle insertion + on-chain ciphertext."
---
...
// result: { signatures, populateProofAccountSignature, createUtxoSignature }
...
// result: { signatures, populateProofAccountSignature, queueSignature, callback?, rentClaim? }
const signatures = result.signatures;
```
...
All four creator factories accept a `CreateStealthPoolNoteArgs` object as the first argument:
...
<ParamField path="options.generationIndex" type="U256">
  Optional deterministic generation index. Defaults to a random `U256` (CSPRNG). Pass the same `generationIndex` on retry to allow the pipeline's `closeProofAccount` step to reclaim a proof-account orphan from a prior failed attempt.
</ParamField>
...
All four creators resolve to a **result object** (not a bare signatures array). The shape depends on the source:

- **From ATA (ZK-only):** `Promise<ATAIntoStealthPoolNoteCreatorResult>` — `{ signatures, populateProofAccountSignature, createUtxoSignature }`. `signatures` (from `OperationOutcome`) lists every tx submitted.
- **From ETA (MPC pipeline):** `Promise<ETAIntoStealthPoolNoteCreatorResult>` — `{ signatures, populateProofAccountSignature, queueSignature, callback?, rentClaim? }`. `queueSignature` queues the Arcium MPC computation; `callback` / `rentClaim` carry the MPC callback + rent-reclaim outcomes when awaited.
...
try {
  const result = await createNote({ destinationAddress: recipient, mint, amount });
  const signatures = result.signatures;
} catch (err) {
