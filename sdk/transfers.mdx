---
title: "Transfers"
description: "An overview of the transfer models in Umbra V14 — unlinkable transfers via Stealth Pool Notes, confidential direct transfers via getTransferorFunction (ETA-to-ETA), and amount-hiding ETA operations."
---
...
- **From a PATA** — tokens enter the pool directly from a `PublicAssociatedTokenAccount`. Single tx, no MPC.
...
- **Self-burnable → PATA** — write a note, then burn it out to a plaintext PATA from any wallet.

(Receiver-burnable → PATA exists on-chain but is not yet exposed in the SDK.)
...
The on-chain program supports several confidential-transfer variants. For direct ETA-to-ETA transfers, use `getTransferorFunction` documented below. For moving funds between PATAs and your own ETAs, use [deposit](/sdk/deposit) / [withdraw](/sdk/withdraw) / [convert](/sdk/conversion).
