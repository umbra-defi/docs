---
title: "Installation"
description: "Install @umbra-privacy/sdk@5.x. V14 ships everything in one package — operations are imported from named subpaths (/registration, /deposit, /burn, /query, …); the ZK prover lives at /zk-prover."
---

## Package

The SDK is a single package — `@umbra-privacy/sdk`. The ZK prover, indexer client, relayer client, store adapters, and Solana primitives are all subpaths of the same package. **There is no separate `@umbra-privacy/web-zk-prover` in V14** (it was folded into `@umbra-privacy/sdk/zk-prover` in the V14 bundle).
...
**Every operation factory lives behind a named subpath.** This keeps the main barrel small and helps bundlers tree-shake aggressively. The subpath map (V14):
...
- `@umbra-privacy/sdk/withdrawal` — ETA → PATA withdrawer.
...
- `@umbra-privacy/sdk/constants` — `BPS_DIVISOR`, mint lists, etc.
...
// ZK prover (V14: same package, named subpath)
import {
...
<Note>
The **burner** ZK prover factories retained V13 spellings (`getClaim…ClaimableUtxo…`). The **creator** prover factories were renamed: `getETAIntoStealthPoolNoteCreatorProver` (ETA-source) and `getATAIntoStealthPoolNoteCreatorProver` (ATA-source). Only the burner names are "legacy".
</Note>
