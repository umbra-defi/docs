...
// 1–2 depending on which sub-steps were needed (V14 splits into two independent on-chain instructions).
```
...
<Note>
The V14 hook slot names are **`initUserAccount`, `registerX25519PublicKey`, `registerAnonymousUsage`** (not `userAccountInitialisation` / `registerUserForAnonymousUsage` from V13). Each is `SkippableDirectTransactionStepHooks` or `SkippableMpcTransactionStepHooks` — `{ onPreSend, onPostSend, onSkipped }` — not the V13 `{ pre, post }` shape.
</Note>

## The V14 registration flow

V14 split the legacy 3-step registration into **two independent on-chain instructions**. Both `init_if_needed` the `EncryptedUserAccount` PDA, so either can run first.
...
<Steps>
  ...
  <Step title="register_user_for_anonymous_usage_v18">
...
**Partial completion**: Registration submits up to two separate transactions (V14 dropped the third). If the second transaction fails or is cancelled, the first is already confirmed on-chain. Call `register()` again — it checks on-chain state and skips the sub-step that already completed.
