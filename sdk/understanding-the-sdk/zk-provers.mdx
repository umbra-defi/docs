...
In V14 the prover and the asset CDN live inside the main SDK at the `@umbra-privacy/sdk/zk-prover` subpath. The standalone `@umbra-privacy/web-zk-prover` package from V13 has been deleted.
...
For PATA-source creation, see `IZkProverForATAIntoStealthPoolNote` below.
...
Required by: `getReceiverBurnableStealthPoolNoteIntoETABurnerFunction` deps (`zkProver`). This prover accepts a batch size `nLeaves` (1–5 in V14 — the SDK chunks to 5 notes per proof).
