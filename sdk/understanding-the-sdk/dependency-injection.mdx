...
interface GetUmbraClientDeps {
  ...
  readonly utxoDataStore?: UtxoDataStore;
...
interface TransactionForwarder {
  forwardSequentially(txs: readonly SignedTransaction[]): Promise<readonly TransactionSignature[]>;
  forwardInParallel(txs: readonly SignedTransaction[]): Promise<readonly TransactionSignature[]>;
  // Sends a single tx without awaiting confirmation (e.g. best-effort rent reclaim).
  fireAndForget(tx: SignedTransaction): Promise<TransactionSignature>;
}
```
...
const jitoForwarder: TransactionForwarder = {
  ...
  fireAndForget: async (tx) => (await submitToJitoBundleEndpoint([tx]))[0],
};
```
