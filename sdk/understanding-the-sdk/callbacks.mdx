...
description: "Per-operation hooks interfaces: DirectTransactionStepHooks, MpcTransactionStepHooks, RegistrationHooks, BurnHooks. Named per-phase and per-step slots scoped to each operation."
---

## Overview

The SDK uses per-operation **hooks** objects (not a single generic `TransactionCallbacks`). Each factory's `deps.hooks` or `options.hooks` slot accepts a named interface specific to that operation, with named per-phase / per-step hook keys. Hooks observe the operation as it moves through validation, fetching, ZK proving, transaction submission, MPC callback waiting, and completion.
...
**Skipped steps fire `onSkipped`, not `onPreSend` / `onPostSend`.** This is how the SDK signals "the step was a no-op because the on-chain state was already correct."

---

## Usage — Registration

Registration uses a single `hooks` option (`RegistrationHooks`) with named per-step slots and top-level phase hooks. Each skippable step slot takes `{ onPreSend, onPostSend, onSkipped }`.
...
Creating a Stealth Pool Note runs through validation, mint fetch, key derivation, ZK proving, and up to two on-chain transaction steps. Hooks are supplied through the factory's `deps.hooks` slot (type `ATAIntoStealthPoolNoteCreatorHooks`), which carries top-level phase hooks plus per-step `DirectTransactionStepHooks` for each transaction. (The same hooks shape is also accepted at call time via `options.hooks`.) The per-step slots are `populateProofAccount` and `createStealthPoolNote` — each a `DirectTransactionStepHooks` with `onPreSend` / `onPostSend`. There is no `createUtxo` or `closeProofAccount` slot.
...
const createNote = getATAIntoSelfBurnableStealthPoolNoteCreatorFunction(
  { client },
  {
    ...
    hooks: {
      ...
      onValidationComplete:      async ({ generationIndex }) => console.log("Generation index:", generationIndex),
      onMintFetchStart:          async () => console.log("Fetching mint..."),
      ...
      onZkProofGenerationStart:  async () => console.log("Proving..."),
      onZkProofGenerationComplete: async ({ elapsedMs }) => console.log("Proof done in", elapsedMs, "ms"),

      // Per-transaction-step hooks (DirectTransactionStepHooks)
      populateProofAccount: {
        onPreSend:  async () => console.log("Creating proof account..."),
        onPostSend: async ({ signature }) => console.log("Proof account:", signature),
      },
      createStealthPoolNote: {
        onPreSend:  async () => console.log("Submitting note write..."),
        onPostSend: async ({ signature }) => console.log("Note written:", signature),
      },

      onComplete: async (result) => console.log("Note created:", result),
      onError:    async ({ phase, error }) => console.error("Failed at", phase, error),
    },
  },
);

await createNote({ destinationAddress: recipient, mint, amount });
```
