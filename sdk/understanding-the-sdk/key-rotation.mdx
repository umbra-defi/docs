...
description: "Offset-based key rotation: U512 offsets on getUmbraClient shift KMAC256 derivation to produce new X25519, MVK, and commitment keys for the same wallet. Complete deps map per factory."
---
...
Each offset is typed as a branded `U512`, not a plain `bigint`. Brand each value with the `assertU512` predicate (there is no `createU512` wrapper) before assigning it — a bare `1n` will not type-check:

```typescript
import { assertU512, type U512 } from "@umbra-privacy/sdk/types";

const u512 = (value: bigint): U512 => {
  assertU512(value, "offset");
  return value;
};
...
const client = await getUmbraClient({
  ...
  rpcSubscriptionsUrl,
  offsets: {
    x25519UserAccountPrivateKey: u512(1n),   // rotate the token encryption key
    poseidonPrivateKey: u512(0n),
    masterViewingKey: u512(0n),
    x25519MasterViewingKeyEncryptingPrivateKey: u512(0n),
    mintX25519PrivateKey: u512(0n),
    rescueCommitmentBlindingFactor: u512(0n),
    randomCommitmentFactor: u512(0n),
  },
});
```
...
- `rescueKeyGenerator`, `rescueEncryptor`, `rescueCommitmentGenerator`.
...
- `accountInfoProvider`.
- `rescueCipherDecryptor` — override to provide a custom Rescue cipher decryption backend.
