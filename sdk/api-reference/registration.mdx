...
<ParamField path="callbacks.userAccountInitialisation" type="TransactionCallbacks">
  `pre(tx)` is called before the account init transaction is sent. `post(tx, sig)` is called after it confirms.
</ParamField>

<ParamField path="callbacks.registerX25519PublicKey" type="TransactionCallbacks">
  `pre(tx)` is called before the X25519 key registration transaction is sent. `post(tx, sig)` is called after it confirms.
</ParamField>

<ParamField path="callbacks.registerUserForAnonymousUsage" type="TransactionCallbacks">
  `pre(tx)` is called before the user commitment registration transaction is sent. `post(tx, sig)` is called after it confirms.
</ParamField>
