...
Moves tokens from the signer's ETA back to the destination's PATA.
...
<ParamField path="options.feeVaultOffset" type="U128" default="0n">
  Fee-vault PDA offset. The fee-vault path is currently unused for public-PATA withdrawals — leave at `0n` (sentinel: no fee-vault interaction).
</ParamField>
...
<ParamField path="options.hooks" type="FromEncryptedTokenAccountIntoPublicAssociatedTokenAccountWithdrawHooks">
...
Tokens are sent to the PATA associated with `destinationAddress` for the given mint. If that PATA does not exist, the transaction will fail. To send to a different address anonymously, use the [Stealth Pool flow](/sdk/mixer/overview).
