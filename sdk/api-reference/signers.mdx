...
**Alias:** `generateKeyPairSigner()` is re-exported as a convenience alias for `createInMemorySigner()`.
...
**Alias:** `getUmbraSignerFromKeypairBytes()` is available as an alternative name for `createSignerFromPrivateKeyBytes()`.
...
**Alias:** `getUmbraSignerFromWalletStandardWallet()` is available as an alternative name for `createSignerFromWalletAccount()`.
...
### `createWalletAdapterSigner` (Deprecated)

<Warning>
This function is deprecated. Use `createSignerFromWalletAccount` with the Wallet Standard API instead. All modern Solana wallets (Phantom, Solflare, Backpack) implement the Wallet Standard directly.
</Warning>
...
Adapts a `@solana/wallet-adapter-base`-style `StandardWalletAdapter` to `IUmbraSigner`. Only use this if your app is locked to the legacy `@solana/wallet-adapter-react` ecosystem and cannot migrate to Wallet Standard.

<ParamField path="adapter" type="WalletAdapterShape" required>
  The raw adapter object from `@solana/wallet-adapter-react`'s `useWallet().wallet.adapter`. The adapter must expose a `wallet` property (the underlying Wallet Standard object).
</ParamField>

---

## `WalletAdapterShape` Interface (Deprecated)
...
If `wallet` is present and supports `solana:signTransaction`, the SDK uses the wire-format signing path. If absent, it falls back to the adapter's `signTransaction` method (legacy path).
