...
description: "Complete reference for getUmbraClient and the IUmbraClient interface."
---
...
<Note>
There is no client-level `commitment` parameter. Instead, each SDK function accepts per-call commitment overrides via its `options` argument (e.g., `accountInfoCommitment` and `epochInfoCommitment`). Both default to `"confirmed"`.
</Note>
...
<ParamField path="deferMasterSeedSignature" type="boolean" default="false">
  When `false` (default): master seed is eagerly derived at construction time — the wallet prompt fires before the returned `Promise` resolves. When `true`: master seed derivation is lazy — the wallet prompt fires on first use.
</ParamField>
...
The client object returned by `getUmbraClient`. All fields are read-only.
...
<ResponseField name="fetchMerkleProof" type="MerkleProofFetcherFunction | undefined">
  Indexer-based single-leaf Merkle proof fetcher. `undefined` if `indexerApiEndpoint` was not provided.
</ResponseField>

<ResponseField name="fetchBatchMerkleProof" type="BatchMerkleProofFetcherFunction | undefined">
  Indexer-based per-batch Merkle proof fetcher. Pass to burner factories via `deps.fetchBatchMerkleProof`. `undefined` if `indexerApiEndpoint` was not provided.
</ResponseField>

<ResponseField name="fetchUtxoData" type="UtxoDataFetcherFunction | undefined">
  Indexer-based Stealth Pool Note data fetcher used internally by the scanner. Field name retained from V13 for backwards compatibility — V14 scanner override is `deps.fetchStealthPoolNoteData`. `undefined` if `indexerApiEndpoint` was not provided.
</ResponseField>

<ResponseField name="fetchTreeSummary" type="TreeSummaryFetcherFunction | undefined">
  Indexer-based tree summary fetcher used by the scanner to discover active trees. `undefined` if `indexerApiEndpoint` was not provided.
</ResponseField>

<ResponseField name="utxoDataStore" type="IUtxoDataStore | undefined">
  Persistent scan-progress + note store. Same instance supplied at construction; `undefined` if not provided.
</ResponseField>

<ResponseField name="nullifierStore" type="INullifierStore | undefined">
  Persistent nullifier store. `undefined` if not provided.
</ResponseField>
