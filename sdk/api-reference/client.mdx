...
<Note>
In v2.0.x, there is no client-level `commitment` parameter. Instead, each SDK function accepts per-call commitment overrides via its `options` argument (e.g., `accountInfoCommitment` and `epochInfoCommitment`). Both default to `"confirmed"`.
</Note>
