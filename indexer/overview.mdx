...
- **Merkle proofs** — generate a Merkle inclusion proof (authentication path) for any leaf. Required to construct a valid burn transaction on-chain. The burner factories fetch a **per-batch** proof immediately before submission, so the scanner does not pre-bundle proofs.
...
<Note>
The service is named `utxo-indexer` for historical reasons. The records it serves are Stealth Pool Notes; the on-chain commitments are still called UTXOs at the protocol level.
</Note>
...
- `client.fetchUtxoData` — Stealth Pool Note data fetcher (`GET /v1/utxos/...`). Field name retains the legacy spelling; the scanner deps override is `fetchStealthPoolNoteData`.
