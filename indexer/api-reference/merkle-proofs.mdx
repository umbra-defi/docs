...
<Warning>
Merkle proofs become **stale** when new leaves are inserted into the tree (because the root changes). The burner factories fetch a per-batch proof immediately before submitting the burn, so simply re-run the burner on a stale-root failure — it refetches the proof. Never cache proofs across operations.
</Warning>
