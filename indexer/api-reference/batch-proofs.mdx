...
Returns multiple Merkle inclusion proofs for the specified insertion indices, all guaranteed to be computed against the same tree root. This is what the V14 burner factory calls per batch — burns that bundle multiple Stealth Pool Notes need all proofs to share a root.
