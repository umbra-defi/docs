...
description: "V14 API reference: getBurnableStealthPoolNoteScannerFunction (zero-arg) + 3 burner factories (receiver→ETA, self→ETA, self→PATA) with fetchBatchMerkleProof, ZK prover, and relayer deps."
---
...
Throws `FetchUtxosError` (named with the legacy noun; class name retained for backwards compatibility — the error itself covers V14 Stealth Pool Note scanning). See [Errors](./errors#fetchutxoserror).
...
- **`relayer`** — adapter built from `getUmbraRelayer({ apiEndpoint })`. `BurnSubmitterFunction` and `BurnStatusPollerFunction` are V14 TypeScript aliases of `ClaimSubmitterFunction` and `ClaimStatusPollerFunction`, so the relayer client's `submitClaim` / `pollClaimStatus` plug in directly under the renamed property names.
...
) => Promise<BurnStealthPoolNoteIntoEncryptedTokenAccountResult>;
...
interface BurnBatchResult {
  ...
  readonly resolvedVariant?: string;           // e.g. "claim_into_existing_shared_balance_v14"
...
Same shape; tokens released to the destination PATA instead of an ETA. `zkProver` is `IZkProverForClaimSelfClaimableUtxoIntoPublicBalance` — use `getClaimSelfClaimableUtxoIntoPublicBalanceProver` from `@umbra-privacy/sdk/zk-prover`.

<Note>
Receiver-burnable → PATA exists on-chain but is not yet shipped in the SDK. Use receiver-burnable → ETA then a regular withdrawal until the SDK adds the direct path.
</Note>
