---
title: "Deposit"
description: "API reference: getATAIntoETADirectDepositorFunction + Stealth Pool Note creators."
---
...
All optional — each category falls back to the corresponding provider on `client`.

```typescript
interface GetATAIntoETADirectDepositorFunctionDeps {
  readonly rpc?: {
    ...
    readonly epochInfo?: GetEpochInfo;                                 // Token-2022 transfer-fee schedule
  };
  readonly arcium?: {
    readonly microLamportsPerAcu?: MicroLamportsPerAcu;                // default 0n (no priority fees)
...
Per-call priority/retry options are not surfaced on the call signature. Priority fees are configured at factory time via `deps.arcium.microLamportsPerAcu`; transport behaviour comes from `deps.rpc.transactionForwarder`.
...
function getETAIntoSelfBurnableStealthPoolNoteCreatorFunction(
  args: { client: IUmbraClient },
  deps: GetETAIntoSelfBurnableStealthPoolNoteCreatorFunctionDeps, // { zkProver: IZkProverForETAIntoStealthPoolNote; … }
...
// ETAIntoStealthPoolNoteCreatorResult (= CreateUtxoFromEncryptedBalanceResult) is an object:
interface ETAIntoStealthPoolNoteCreatorResult {
  readonly signatures: readonly TransactionSignature[]; // every tx submitted, in order
  readonly populateProofAccountSignature: TransactionSignature;
  readonly queueSignature: TransactionSignature;
  readonly callback?: CallbackOutcome;
  readonly rentClaim?: RentClaimOutcome;
}
...
// ATAIntoStealthPoolNoteCreatorResult (= CreateUtxoFromPublicBalanceResult) is an object:
interface ATAIntoStealthPoolNoteCreatorResult {
  readonly signatures: readonly TransactionSignature[]; // every tx submitted, in order
  readonly populateProofAccountSignature: TransactionSignature;
...
- `hooks?: ATAIntoStealthPoolNoteCreatorHooks` — per-phase + per-step lifecycle hooks. There are exactly two step slots: `populateProofAccount` and `createStealthPoolNote` — each typed `DirectTransactionStepHooks` with `{ onPreSend, onPostSend }` (plus `interceptInstructions`, `onTransactionBuilt`, `onTransactionSigned`). There is no `closeProofAccount` slot and no `onSkipped` (these steps are not skippable).
