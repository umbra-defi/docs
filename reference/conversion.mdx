...
- `hooks?: ConvertToSharedHooks` — overall batch lifecycle hooks; per-mint MPC-step hooks (`onPreSend`, `onPostSend`, `onMonitorStarted`, `onFinalized`) live in the `perMintTransaction` member.
...
- `optionalData?: OptionalData32` - 32-byte caller metadata. Pre-hashed only.
- `hooks?: RotateMintKeyHooks` - per-phase lifecycle hooks for the MPC re-encryption.
- `microLamportsPerAcu?: MicroLamportsPerAcu` - optional priority fee per Arcium compute unit.
- `options?: RotateMintKeyOptions` - per-call options (e.g. skip the pre-flight key-consistency assertion).
