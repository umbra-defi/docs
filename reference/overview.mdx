---
title: "SDK Reference"
description: "Complete V14 SDK reference index: all factory functions and the subpath each lives under (registration, deposit, withdrawal, burn, query, conversion, compliance, account, zk-prover, store-adapters)."
---

This reference covers the full public API surface of `@umbra-privacy/sdk@5.x` (V14). Pages are organised by feature area — each section documents factory functions alongside their parameter types, result types, and associated errors.
...
| `@umbra-privacy/sdk/deposit` | Direct PATA→ETA deposit + 4 Stealth Pool Note creators (PATA/ETA × self/receiver) |
...
- [Deposit](/reference/deposit) — Direct PATA→ETA deposit + 4 Stealth Pool Note creators.
- [Withdrawal](/reference/withdraw) — Direct ETA→PATA withdrawal.
