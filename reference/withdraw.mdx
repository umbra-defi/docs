---
title: "Withdrawal"
description: "API reference: getETAIntoATAWithdrawerFunction returns WithdrawResult with queueSignature and callbackSignature via Arcium MPC."
---
...
```typescript
interface GetETAIntoATAWithdrawerFunctionDeps {
  readonly rpc?: {
  ...
  readonly random?: {
    readonly generateRandomU64?: () => U64;                            // default `computationOffset`
    readonly generateRandomU128?: () => U128;                          // default `mpcCallbackDataOffset` / `feeVaultOffset`
  };
}
```
...
There are no `priorityFees`, `awaitCallback`, `skipPreflight`, `maxRetries`, or `epochInfoCommitment` options on the withdrawer.
