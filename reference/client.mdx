...
<Note>
In v2.0.x, there is no client-level `commitment` parameter. Each SDK function accepts per-call commitment overrides via its `options` argument (e.g., `accountInfoCommitment`, `epochInfoCommitment`, or `commitment`). Both default to `"confirmed"`.
</Note>
...
- `indexerApiEndpoint?: string` - Base URL for the Umbra indexer (e.g. `"https://indexer.api.umbraprivacy.com"`). Required for mixer operations (`getClaimableUtxoScannerFunction`).
