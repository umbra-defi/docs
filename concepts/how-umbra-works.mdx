...
1. Your tokens move from your **PublicAssociatedTokenAccount (PATA)** to the on-chain pool custody account.
...
<Note>
This is why Umbra MPC operations take a few seconds longer than a standard token transfer — there is an off-chain MPC round-trip between the two on-chain instructions. Pure ZK operations (e.g. a PATA-source note write) are single-tx and finish in normal Solana confirmation time.
</Note>
