...
Solana uses **AssociatedTokenAccounts (ATAs)** to hold SPL and Token-2022 tokens. These are fully public; anyone can query the balance.

Umbra introduces **EncryptedTokenAccounts (ETAs)** — on-chain PDA accounts that hold your token balance in encrypted form.

**ATA (standard public ATA)**
...
switch (result?.state) {
  case "shared":
    console.log("Balance:", result.balance); // decrypted U64
    break;
  case "mxe":
...
<Warning>
Only Shared-mode ETAs can be decrypted client-side. MXE-mode ETAs return `{ state: "mxe" }` without a balance. Use the [converter](/sdk/conversion) (`getNetworkEncryptionToSharedEncryptionConverterFunction`) to move to Shared mode.
</Warning>
...
## Lottery Points (Umbra Confidential SPL)

**Umbra Confidential SPL** adds a loyalty/lottery-point tracking layer to every `EncryptedTokenAccount`. It is live on-chain and in active SDK integration.
...
ETAs deployed before Umbra Confidential SPL have zero-byte placeholders for the two new fields. To enable points accrual, each such account must go through a two-step migration:
...
2. **`initialise_points_for_network_balance_v17`** or **`initialise_points_for_shared_balance_v17`** — an Arcium MPC instruction that overwrites the placeholder bytes with valid encrypted-zero ciphertexts and sets `is_points_initialised = true`.
