...
A **Stealth Pool Note** is Umbra V14's name for what V13 called a "UTXO". On-chain it is still a [UTXO](https://bitcoin.org/bitcoin.pdf)-style commitment leaf in the mixer's Indexed Merkle Tree — the rename happens at the SDK surface. Everywhere the SDK used to say "UTXO" we now say "Stealth Pool Note"; everywhere it used to say "claim" we now say "burn".
...
<Steps>
  ...
  <Step title="Scan">
    The SDK queries the indexer for note ciphertexts, attempts to decrypt each with your viewing keys, and groups successful decrypts by `(burn kind, source)`. The V14 scanner is **zero-arg** — it walks every active tree and tracks its own cursor in `client.utxoDataStore`. See [Scanning](/sdk/mixer/fetching-utxos).
  </Step>
  <Step title="Burn">
    ...
    Without revealing *which* commitment it is. The on-chain program verifies the proof, burns the nullifier, and releases the tokens to the destination ETA or PATA.
  </Step>
</Steps>
...
- **Self-burnable from PATA** — `getATAIntoSelfBurnableStealthPoolNoteCreatorFunction` — sender keeps the unlocker; funded from the writer's PATA. Single tx, no MPC.
...
- **Receiver-burnable from PATA** — `getATAIntoReceiverBurnableStealthPoolNoteCreatorFunction` — unlocker encrypted against the recipient's `userCommitment`; funded from the writer's PATA.
...
- Combining a burn into an ETA (rather than a PATA) hides the destination amount further.
...
The V14 scanner walks every active tree automatically — you do not need to specify a tree index. (V13's `(treeIndex, start, end)` arguments are gone; cursor state lives in `client.utxoDataStore` and is per-tree under the hood.)
