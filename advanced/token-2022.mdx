...
description: "Token-2022 transfer fee handling: epoch-based fee schedule detection, fee deduction before protocol fees, epochInfoCommitment override. Affects deposit and public UTXO creation."
---
...
The amount credited to your encrypted balance is `actualReceived - protocolFees`.
...
- `getPublicBalanceToEncryptedBalanceDirectDepositorFunction` - fetches epoch info only when a Token-2022 transfer fee is detected
- `getPublicBalanceToSelfClaimableUtxoCreatorFunction` - same
- `getPublicBalanceToReceiverClaimableUtxoCreatorFunction` - same

Operations that draw from an encrypted balance rather than a public ATA are not affected by Token-2022 transfer fees, because the transfer happens within the Umbra program and does not cross the Token-2022 transfer fee boundary.

Each of these functions accepts an `epochInfoCommitment` option (defaults to `"confirmed"`) to control the commitment level used when fetching the current epoch for fee schedule selection.
